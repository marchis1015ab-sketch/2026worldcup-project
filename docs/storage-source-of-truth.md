...
| 새 메인 타임라인 블록 | `app.js`에서 `wc26_new_suit_timeline_blocks_v1` localStorage 직접 읽기 | `app.js`에서 `wc26_new_suit_timeline_blocks_v1` 및 backup prefix로 직접 쓰기 | 아니오 | 예. localStorage 단독 | 현재 브라우저 localStorage가 유일 원본 | 위험 | `wc26_new_suit_timeline_blocks_v1` | 다른 PC와 공유 보장 없음. 운영 전 공유 데이터로 오해하지 않도록 표시 필요 | Supabase/shared_state 등 서버 단일 경로로 별도 이관 필요 |
