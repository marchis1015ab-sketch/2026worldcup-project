...
function normalizeMenuLaunchSection(targetId = "", sectionId = "") {
  ...
  if (normalizedTarget === "match-schedule") {
    return isMobileSectionViewport() ? "group-stage" : "bracket";
  }
...
function stabilizeMobileHashSectionRestore(options = {}) {
  if (!isMobileSectionViewport()) {
    return;
  }
  const hash = String(window.location.hash || "").trim();
  ...
  const maxPasses = Math.max(1, Number(options.maxPasses) || 16);
  const intervalMs = Math.max(60, Number(options.intervalMs) || 140);
  let remaining = maxPasses;
  const applyRestore = () => {
    if (restoreMobileSectionFromHash()) {
      return true;
    }
    return forceMobileHashViewFallback();
  };
  applyRestore();
...
function focusSection(targetId, sectionId) {
  ...
  if (targetId === "match-schedule") {
    if (isMobileSectionViewport() && sectionId === "group-stage") {
...
function centerMatchScheduleToolbarButton(button) {
  if (!isMobileSectionViewport() || !(button instanceof HTMLElement)) {
    return;
  }
  window.requestAnimationFrame(() => {
...
function resolveLaunchSectionForTarget(targetId = "", sectionId = "") {
  const normalizedTarget = String(targetId || "").trim();
  if (!normalizedTarget) {
    return "";
  }
...
function getArchiveSuitDateSortStamp(item = {}) {
  const rawDate = String(item.date || item.createdAt || item.updatedAt || item.timestamp || "").trim();
  if (!rawDate) return 0;
  const parsed = Date.parse(rawDate);
  return Number.isFinite(parsed) ? parsed : 0;
}

function sortArchiveSuitItemsForMobile(tab = storageBridgeSection, items = []) {
  const normalized = normalizeStorageBridgeSection(tab);
  if (normalized !== "video") {
    return Array.isArray(items) ? items : [];
  }
  return [...items].sort((left, right) => {
    const stampDiff = getArchiveSuitDateSortStamp(right) - getArchiveSuitDateSortStamp(left);
    if (stampDiff) return stampDiff;
    const rightDate = String(right?.date || right?.createdAt || "");
    const leftDate = String(left?.date || left?.createdAt || "");
...
function isArchiveSuitMobileDocumentLayout(tab = storageBridgeSection, options = {}) {
  if (options.editMode || options.deleteMode) return false;
  if (normalizeStorageBridgeSection(tab) === "gallery") return false;
  return window.matchMedia("(max-width: 767px)").matches;
}

function getArchiveSuitMobileBadgeLabel(tab = storageBridgeSection, item = {}) {
  const normalized = normalizeStorageBridgeSection(tab);
  const fileKind = getArchiveSuitFileKind(item);
  if (fileKind === "video" || normalized === "video") return "영상";
  if (normalized === "file-storage") return "파일";
  return "문서";
}
...
function renderArchiveSuitInlineDetail(item = {}, tab = storageBridgeSection) {
  ...
  const typeLabel = escapeTimelineHtml(item.fileType || getArchiveSuitMobileBadgeLabel(tab, item));
  let preview = `<div class="archive-suit-preview-unavailable">미리보기를 표시할 수 없습니다. 파일을 다운로드하거나 다시 확인해 주세요.</div>`;
  if (fileUrl && fileType.startsWith("image/")) {
    preview = `<img class="archive-suit-preview-media" src="${fileUrl}" alt="${fileName}">`;
  } else if (fileUrl && fileType.startsWith("video/")) {
    preview = `<video class="archive-suit-preview-media" src="${fileUrl}" controls playsinline></video>`;
  } else if (fileUrl && fileType.startsWith("audio/")) {
    preview = `<audio class="archive-suit-preview-audio" src="${fileUrl}" controls></audio>`;
  } else if (fileUrl && fileType === "application/pdf") {
    preview = `<iframe class="archive-suit-preview-frame" src="${fileUrl}" title="${fileName} 미리보기"></iframe>`;
  } else if (fileUrl && (fileType.startsWith("text/") || /\.(txt|csv|json|md)$/i.test(item.fileName || ""))) {
    preview = `<iframe class="archive-suit-preview-frame" src="${fileUrl}" title="${fileName} 텍스트 미리보기"></iframe>`;
  }
  return `<div class="archive-suit-inline-detail">
    <div class="archive-suit-inline-detail__meta">${[date, typeLabel, fileSize].filter(Boolean).join(" / ")}</div>
    ${memo ? `<div class="archive-suit-inline-detail__memo">${memo}</div>` : ""}
    <div class="archive-suit-inline-detail__actions">
      ${fileUrl ? `<a class="archive-suit-download" href="${fileUrl}" download="${fileName}">다운로드</a>` : ""}
    </div>
    <div class="archive-suit-inline-detail__preview">${preview}</div>
  </div>`;
}

function getArchiveSuitMobileSectionCopy(tab = storageBridgeSection) {
  const normalized = normalizeStorageBridgeSection(tab);
  if (normalized === "file-storage") {
    return {
      eyebrow: "FILE INDEX",
      title: "파일 보관 목록",
      description: "작업 파일과 첨부자료를 문서철처럼 한 줄씩 빠르게 확인합니다.",
    };
  }
  if (normalized === "video") {
    return {
      eyebrow: "VIDEO INDEX",
      title: "동영상 보관 목록",
      description: "영상 원본과 전달본을 제목, 날짜, 메모 중심으로 정리합니다.",
    };
  }
  return {
    eyebrow: "DOCUMENT INDEX",
    title: "문서 보관 목록",
    description: "모바일에서는 문서 제목과 메모를 먼저 보고, 필요한 항목만 펼쳐서 확인합니다.",
  };
}

function renderArchiveSuitMobileDocumentHeader(tab = storageBridgeSection, items = []) {
  const copy = getArchiveSuitMobileSectionCopy(tab);
  const normalized = normalizeStorageBridgeSection(tab);
  const countLabel = `${items.length}건`;
  const tabLabel = escapeTimelineHtml(ARCHIVE_SUIT_TABS[normalized]?.label || copy.title);
  return `<section class="archive-suit-mobile-overview" aria-label="${tabLabel} 모바일 개요">
    <div class="archive-suit-mobile-overview__eyebrow">${copy.eyebrow}</div>
    <div class="archive-suit-mobile-overview__header">
      <div>
        <h4 class="archive-suit-mobile-overview__title">${copy.title}</h4>
        <p class="archive-suit-mobile-overview__description">${copy.description}</p>
      </div>
      <div class="archive-suit-mobile-overview__count">${countLabel}</div>
    </div>
  </section>`;
}

function renderArchiveSuitMobileEmptyState(tab = storageBridgeSection) {
  const normalized = normalizeStorageBridgeSection(tab);
  const copy = getArchiveSuitMobileSectionCopy(normalized);
  return `<section class="archive-suit-empty archive-suit-empty--mobile" aria-label="${copy.title} 빈 상태">
    <div class="archive-suit-empty__badge">${copy.eyebrow}</div>
...
function sortArchiveSuitGalleryGroupsByDate(groups = []) {
...
function renderArchiveSuitGalleryDateSections(groups = []) {
  const sortedGroups = sortArchiveSuitGalleryGroupsByDate(groups);
...
function renderArchiveSuitGalleryList(options = {}) {
  ...
  if (options.editMode || options.deleteMode) {
    return `<div class="archive-suit-list archive-suit-gallery-grid">${sortArchiveSuitGalleryGroupsByDate(groups)
      .map((group) => renderArchiveSuitGalleryCard(group, options))
      .join("")}</div>`;
  }
  ...
  return `${mobileHeader}<div class="${listClassName}">${items
    .map((item) => {
      ...
      const deleteBox = options.deleteMode
        ...
        : "";
      const editButton = options.editMode ? `<button type="button" class="archive-suit-inline-btn" data-archive-edit-item="${id}">수정</button>` : "";
      if (useMobileDocumentLayout) {
        const badgeLabel = getArchiveSuitMobileBadgeLabel(normalized, item);
        const metaText = [date, fileSize].filter(Boolean).join(" / ") || escapeTimelineHtml(badgeLabel);
        const isCompactMobileLine = normalized === "file-storage" || normalized === "document-storage" || normalized === "video";
        return `<article class="archive-suit-item archive-suit-item--document${selected ? " is-selected" : ""}" data-archive-item-id="${id}">
          ${deleteBox}
          <div class="archive-suit-item-shell${isCompactMobileLine ? " archive-suit-item-shell--compact-line" : ""}">
            ${isCompactMobileLine ? "" : `<div class="archive-suit-item-badge" aria-hidden="true">${badgeLabel}</div>`}
            <div class="archive-suit-item-main archive-suit-item-main--line${isCompactMobileLine ? " archive-suit-item-main--compact-line" : ""}">
              <div class="archive-suit-item-name">${fileName}</div>
              ${isCompactMobileLine ? "" : `<div class="archive-suit-item-meta">${metaText}</div>`}
            </div>
            <div class="archive-suit-item-toggle">${selected ? "접기" : "열기"}</div>
            ${editButton}
          </div>
...
function renderArchiveSuitPanels() {
  const activeItems = getArchiveSuitDisplayItems(storageBridgeSection);
  const isMobileEmptyState = isArchiveSuitMobileDocumentLayout(storageBridgeSection) && activeItems.length === 0 && archiveSuitMode === "view";
  if (archiveSuitToolbar) {
    archiveSuitToolbar.classList.toggle("is-mobile-empty", isMobileEmptyState);
  }
  if (archiveSuitActionButtons.length) {
    archiveSuitActionButtons.forEach((button) => {
      const action = button.dataset.archiveAction || "";
      button.hidden = false;
      button.disabled = activeItems.length === 0 && action !== "create";
    });
  }
  archiveSuitPanels.forEach((panel) => {
...
function syncMatchShellUi() {
  ...
  // [Z-TEST-END] 3위순위 임시 테스트
  renderMatchResultsPanel();
  centerMatchScheduleToolbarButton(
    document.querySelector('#view-match-schedule .schedule-bridge-toolbar--primary .schedule-bridge-button.is-active'),
  );
  centerMatchScheduleToolbarButton(matchSquadToolbar?.querySelector(".schedule-bridge-button.is-active"));
}
...
function initStorageBridge() {
  ...
  storageBridgeFrameShell?.addEventListener("click", (event) => {
    ...
    const emptyCreateButton = event.target?.closest?.("[data-archive-empty-create]");
    if (emptyCreateButton) {
      event.preventDefault();
      event.stopPropagation();
      closeArchiveSuitCreateMenu();
      setArchiveSuitMode("create", emptyCreateButton.dataset.archiveEmptyTarget || storageBridgeSection);
      return;
    }
    ...
    if (itemCard && archiveSuitMode === "view") {
      const nextId = itemCard.dataset.archiveItemId || "";
      if (isArchiveSuitMobileDocumentLayout(storageBridgeSection)) {
        archiveSuitSelectedId = archiveSuitSelectedId === nextId ? "" : nextId;
      } else {
        archiveSuitSelectedId = nextId;
      }
      renderArchiveSuitPanels();
      return;
    }
...
function setMatchScheduleSection(sectionId) {
  ...
  if (normalized === "bracket" || normalized === "tournament") {
    if (!WC26_MOBILE_MEDIA_QUERY.matches) {
      restoreCurrentOrDefaultMatchScheduleSelection();
      return;
    }
    setMatchShellMode("bracket", {
      knockoutStage: normalizeKnockoutStage(matchBridgeKnockoutStage),
    });
    return;
  }
  if (normalized === "match-schedule" || !normalized) {
    restoreCurrentOrDefaultMatchScheduleSelection();
    return;
  }
...
function activateDirect(button) {
  ...
  const sectionId = resolveLaunchSectionForTarget(targetId, button ? button.dataset.section || "main" : "");
...
function activateSubItem(button) {
  const targetId = button ? button.dataset.target : "";
  const sectionId = resolveLaunchSectionForTarget(targetId, button ? button.dataset.section : "");
...
function activateQuickAction(button) {
  ...
  focusSection(action.target, resolveLaunchSectionForTarget(action.target, action.section));
...
function getDashboardPanelLaunchSection(panel) {
  ...
  if (explicitSection) {
    return resolveLaunchSectionForTarget(targetId, explicitSection);
  }
  return resolveLaunchSectionForTarget(targetId, WC26_DASHBOARD_PANEL_SECTION_DEFAULTS[targetId] || "");
}
...
window.addEventListener("hashchange", () => {
  stabilizeMobileHashSectionRestore();
  if (restoreMobileSectionFromHash()) {
    return;
  }
